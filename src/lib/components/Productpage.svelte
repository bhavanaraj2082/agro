<script>
  ...
  // Make records reactive to data changes
  ...
  $: productStock = data?.records || [];
  $: selectedStockId = productStock?.[0]?.stockId?.[0] || "NA";
  
  // Reset quantity when data changes
  $: if (data) {
    quantity.set(1);
  }
...
{#key data}
  {#if records && records.length > 0}
    {#each records as product, index (product.productId || product.id || index)}
      <div
